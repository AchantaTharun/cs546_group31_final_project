...
export const adminLogin = async (emailAddress, password) => {
  if (!emailAddress || !password)
    throw "Both email Address and Password has to be provided";

  //Initial modifications
  if (typeof emailAddress !== "string")
    throw "Email address is not of valid data type";
...
export const passwordChange = async (
  ...
  if (!emailAddress || !oldPassword || !newPassword || !confirmPassword)
...
export const statusChange = async (status, id, type, reason) => {
  try {
    ...
    if (typeof reason !== "string" || reason.trim() === "")
      throw "Reason is not reasonable";

    //Initial validations
    if (status !== "approved" && status !== "rejected")
      throw "Status has to be either Accepted or Rejected";
    id = help.checkId(id);
    if (type !== "trainer" && type !== "gym")
      throw "Type has to be either trainer or gym in this very case scenario";

    if (type === "trainer") {
      object = await Trainer.findByIdAndUpdate(
        id,
        { status: status },
        { new: true }
      );
    } else if (type === "gym") {
      object = await Gym.findByIdAndUpdate(
        id,
        { status: status },
        { new: true }
      );
    ...
    throw "No Trainer Found";
  }
};

export const getAllRequestReports = async () => {
  const reportRequests = await RejectedRequest.find({})
    .select("_id requestType email phone reason")
    .sort({ rejectedAt: "asc" })
    .exec();
  if (!reportRequests) {
...
export const getAllSignUpRequests = async () => {
  const signUpRequests = await SignUpRequest.find({})
    .select("-_id requestType requestedBy")
    .sort({ createdAt: "asc" })
    .exec();
  if (!signUpRequests) {
...
export const getOneSignUpRequest = async (id) => {
  ...
  const signUpRequest = await SignUpRequest.findOne({ _id: id })
    .select("_id requestType requestedBy")
    .exec();
  if (!signUpRequest) {
...
export const createEvent = async (
  ...
  if (
    ...
    !zipCode ||
    !maxCapacity ||
    (!priceOfAdmission && priceOfAdmission !== 0) ||
    !eventDate ||
    !startTime ||
    !endTime ||
    (!totalNumberOfAttendees && totalNumberOfAttendees !== 0)
  )
  ...
  if (
    ...
    throw "Max capacity value is invalid";
  if (typeof priceOfAdmission !== "number" || priceOfAdmission < 0)
    throw "Price of Admission has an invalid value";
  ...
  if (!help.isEarlierInSameDay(new Date(startTime), new Date(endTime)))
    throw "Start time and End time should be on the same day";
  if (!help.isEarlierInSameDay(new Date(startTime), new Date(eventDate)))
    throw "Event Date and Start time should be on the same day";
  if (new Date() > new Date(eventDate))
    throw "You cannot have an event date and time in the past";
  if (new Date(eventDate) > new Date(startTime))
    throw "You cannot have an event Start date and time in the past";
  if (new Date(startTime) >= new Date(endTime))
    throw "You cannot have an event date and time in the past";

  if (
    typeof totalNumberOfAttendees !== "number" ||
    totalNumberOfAttendees > maxCapacity
  )
    throw "Number of attendees are invalid";
...
export const getOne = async (type, id) => {
  ...
  if (typeof type !== "string")
    throw "Collection type must be in string format";
  type = type.trim();
  if (
    type !== "user" &&
    type !== "trainer" &&
    ...
    throw "Invalid Collection type was passed";
  id = help.checkId(id);
...
export const getSome = async (type, name) => {
  ...
  if (type === "user") {
    ...
    object = await Trainer.find({ trainerName: name })
      .select("-password")
      .exec();
  }
  if (type === "gym") {
...
export const deleteOne = async (type, id) => {
  ...
  if (type === "user") {
    ...
    sendEmail(
      object.email,
      "eliminated",
      "You have broken the application's policy."
    );
  } else if (type === "trainer") {
    ...
    if (object.status === "pending") {
      deletedSignUpRequest = await SignUpRequest.findOneAndDelete({
        requestedBy: id,
      });
      if (!deletedSignUpRequest)
        throw `${type} type sign up request, by the id: ${id} could not be deleted`;
    }
  }
  if (type === "gym") {
